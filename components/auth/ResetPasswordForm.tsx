...
export function ResetPasswordForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setError(null);
    if (!token) {
      setError("Brak tokenu w linku. Poproś o nowy reset hasła.");
      return;
    }
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Nie udało się zresetować hasła.",
      );
    }
  }
    ...
    >
      {!token ? (
        <p role="alert" className="border-l-2 border-brand bg-brand/10 px-4 py-3 text-sm">
          Link jest niekompletny. Użyj odnośnika z e-maila lub{" "}
          <Link href="/zapomniane-haslo" className="underline">
            wyślij nową prośbę
          </Link>
          .
        </p>
